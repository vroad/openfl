...
class Shader {
	...
	private function __disableGL ():Void {
		...
		for (parameter in __paramBool) {
			...
			gl.disableVertexAttribArray (parameter.getAttributeLocation ());
	...
	private function __enableGL ():Void {
		...
		for (input in __inputBitmapData) {
			...
			gl.uniform1i (input.getUniformLocation (), textureCount);
			textureCount++;
			...
			if (glProgram != null) {
				...
				for (input in __inputBitmapData) {
					...
					if (__isUniform.get (input.name)) {
						...
						input.index = ShaderInputIndex.UniformLocation (gl.getUniformLocation (glProgram, input.name));
						
					} else {
						
						input.index = ShaderInputIndex.AttributeLocation (gl.getAttribLocation (glProgram, input.name));
				...
				for (parameter in __paramBool) {
					...
					if (__isUniform.get (parameter.name)) {
						...
						parameter.index = ShaderInputIndex.UniformLocation (gl.getUniformLocation (glProgram, parameter.name));
						
					} else {
						
						parameter.index = ShaderInputIndex.AttributeLocation (gl.getAttribLocation (glProgram, parameter.name));
	...
	private function __updateGL ():Void {
		...
		for (parameter in __paramBool) {
			...
			var location = parameter.getUniformLocation ();
			...
			if (value != null) {
				...
				gl.enableVertexAttribArray (parameter.getAttributeLocation ());
			...
			if (value != null) {
				...
				switch (parameter.type) {
					...
					case MATRIX2X2:
						...
						gl.uniformMatrix2fv (location, false, __uniformMatrix2);
					...
					case MATRIX3X3:
						...
						gl.uniformMatrix3fv (location, false, __uniformMatrix3);
					...
					case MATRIX4X4:
						...
						gl.uniformMatrix4fv (location, false, __uniformMatrix4);
